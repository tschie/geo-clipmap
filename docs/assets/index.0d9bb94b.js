...
`;var ie="/geo-clipmap/assets/grass.e270160d.png";const L=7,l=Math.pow(2,L+1)+2,M=12,a=document.querySelector("canvas"),n=new T({canvas:a});n.setPixelRatio(window.devicePixelRatio);n.outputEncoding=U;n.toneMapping=k;n.toneMappingExposure=.5;const u=new G,s=new j(75,a.offsetWidth/a.offsetHeight,.1,1e6);s.position.set(0,3e3,0);const g=new _(s,n.domElement);g.dragToLook=!0;g.movementSpeed=2e3;g.rollSpeed=1;const d=new H;d.scale.setScalar(45e4);u.add(d);const c={turbidity:{value:10},rayleigh:{value:3},mieCoefficient:{value:.005},mieDirectionalG:{value:.7},elevation:{value:35},azimuth:{value:180},exposure:n.toneMappingExposure},m=new x,re=w.degToRad(90-c.elevation.value),ae=w.degToRad(c.azimuth.value);m.setFromSphericalCoords(1,re,ae);c.sunPosition={value:m};d.material.uniforms=p(p({},d.material.uniforms),c);n.toneMappingExposure=c.exposure;const E=new V(l/(l-2),l/(l-2),l,l);E.rotateX(-Math.PI/2);const h=X.loadTexture(ie);h.wrapS=b;h.wrapT=b;const P=new Z({uniforms:{grass:{type:"t",value:h},gridSize:{value:l},sun:{value:m}},vertexShader:oe,fragmentShader:te}),f=new Q(E,P,M);u.add(f);const W=Y();document.body.appendChild(W.domElement);const N=new B,D={update:!0},C=N.addFolder("Clip Map");C.add(D,"update",!0);C.open();const I=N.addFolder("Material");I.add(P,"wireframe",!1);I.open();const O=()=>{a.width=a.offsetWidth,a.height=a.offsetHeight,s.aspect=a.offsetWidth/a.offsetHeight,s.updateProjectionMatrix(),n.setSize(window.innerWidth,window.innerHeight)};window.addEventListener("resize",O);O();const ne=new K,F=()=>{if(requestAnimationFrame(F),g.update(ne.getDelta()),D.update){for(let o=0;o<M;o++){const e=Math.pow(2,o+L+1);f.setMatrixAt(o,new $().compose(new x(Math.floor(s.position.x),0,Math.floor(s.position.z)),new J,new x(e,1,e)))}f.instanceMatrix.needsUpdate=!0,f.material.needsUpdate=!0}n.render(u,s),W.update()};F();
